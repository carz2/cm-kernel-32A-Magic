...
static int battery_charging_ctrl(batt_ctl_t ctl)
{
	...
	case ENABLE_FAST_CHG:
		...
		break;
	default:
		printk(KERN_ERR "%s: Not supported battery ctr called.!\n");
		result = -EINVAL;
		break;
	}
...
int htc_cable_status_update(int status)
{
	...
	if (status < CHARGER_BATTERY || status > CHARGER_AC) {
		printk(KERN_ERR "%s: Not supported cable status received!", __FUNCTION__);
		return -EINVAL;
	}
	mutex_lock(&htc_batt_info.lock);
...
static int htc_battery_probe(struct platform_device *pdev)
{
	...
	*/
	...
	for (i = 0; i < ARRAY_SIZE(htc_power_supplies); i++) {
		...
		if (rc)
			printk(KERN_ERR "%s: Failed to register power supply (%d)\n", rc);	
	}
